...
class LoginVentana:
    ...
    def mostrar_menu(self, usuario_nombre):
        menu = InterfazMenu(usuario_nombre)
        menu.mostrar_interfaz()

    def procesar_login(self):
        nombre_completo = self.entry_nombre.get().strip() + " " + self.entry_apellido.get().strip()
        correo = self.entry_correo.get().strip()
        usuario_menu = nombre_completo.capitalize()

        if not nombre_completo.strip() or not correo:
        ...
        if usuario:
            ...
            self.master.destroy()  # Cierra la ventana del login
            self.mostrar_menu(usuario_menu)
        else:
            messagebox.showerror("Error", mensaje)
def main():
    root = tk.Tk()
    sistema_autenticacion = SistemaAutenticacion()
    app_login = LoginVentana(root, sistema_autenticacion)
    root.mainloop()
